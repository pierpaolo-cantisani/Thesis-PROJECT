...
M_list <- list()
M_list[[1]] <- read.csv(file.path(PATH, "Dataset_0", "3_Benchmark", "DM_sites_Met1.csv"))
M_list[[2]] <- read.csv(file.path(PATH, "Dataset_0", "3_Benchmark", "DM_sites_Met2.csv"))
M_list[[3]] <- read.csv(file.path(PATH, "Dataset_0", "3_Benchmark", "DM_sites_Met3.csv"))
M_list[[4]] <- read.csv(file.path(PATH, "Dataset_0", "3_Benchmark", "DM_sites_Met4.csv"))
M_list[[5]] <- read.csv(file.path(PATH, "Dataset_0", "3_Benchmark", "DM_sites_Met5.csv"))
...
write_xlsx(Upset_genes_df, file.path(PATH, "Dataset_0", "4_Integration_results", "Upset_genes.xlsx"))
...
create_DM_matrix <- function(Method_df, M_matrix, DE_matrix) {
  ...
  attr(Matrix_M, "coord_key") <- Method_df$coord_key #saving coord_key for final comparison with Ground Truth
...
M4_df <- read.csv(file.path(PATH, "Dataset_0", "3_Benchmark", "DM_sites_Met4.csv"))
M5_df <- read.csv(file.path(PATH, "Dataset_0", "3_Benchmark", "DM_sites_Met5.csv"))
...
#Collapsing the matrix into 1 col x sample (case-ctrl)
n_pairs <- 50    #case and ctrl paired
Matrix_Mv_new <- sapply(seq_len(n_pairs), function(k) Matrix_Mv[, k + n_pairs] - Matrix_Mv[, k])
...
#Collapsing the expression matrix into 1 col x sample (case-ctrl)
...
for(m in seq_along(Matrix_exp)) {
  ...
  res$SYMBOL_base <- sub("\\.\\d+$", "", res$SYMBOL)     # gene puro
  res$coord_key <- attr(Matrix_Mval[[m]], "coord_key") # CpG (identificatore vero)
...
for(m in seq_along(Matrix_exp_Q)) {
  ...
  res$SYMBOL_base <- sub("\\.\\d+$", "", res$SYMBOL)     # gene puro
  res$coord_key <- attr(Matrix_Mval_Q[[m]], "coord_key") # CpG (identificatore vero)
...
ctrl_mean <- rowMeans(Matrix_Mv_2[, 1:50, drop = FALSE])
case_mean <- rowMeans(Matrix_Mv_2[, 51:100, drop = FALSE])
...
write.csv(Stats_table, file.path(PATH, "Dataset_0", "4_Integration_results", "Stats_table.csv"), row.names = FALSE)

dev.off()



## Importing output table
