...
### Dataset CTRL positivo. Devo ottenere: 
#   - hyp_up_hypo e hyp_down_hyper significativi
#   - hyp_up_hyper e hyp_down_hypo non significativi
#   - Sezioni 4-5 causali. probabilmente non significativi
...
RNAseq_universe <- read.csv(file.path(PATH, "Dataset_0", "1_RNA-Seq", "RNAseq_universe.csv"), col.names = c("SYMBOL", "log2FoldChange", "padj"))
...
for(METHOD in 1:5) {
  ...
  #Importing M-values
  Scalar_M <- read.xlsx(file.path(PATH, "Dataset_0", "4_Integration_results", "Method_final_df.xlsx"), sheet = sprintf("M%d", METHOD))
...
Stats_table <- read.csv(file.path(PATH, "Dataset_0", "4_Integration_results", "Stats_table.csv"))
...
#Writing on file
write.xlsx(Stats_table_final, file.path(PATH, "Dataset_0", "4_Integration_results", "Stats_table_final.xlsx"), rowNames = FALSE)


##Plotting graphs:
pdf(file.path(PATH, "Dataset_0", "4_Integration_results", "Additional_comparison D0.pdf"), height = 10, width = 15)
