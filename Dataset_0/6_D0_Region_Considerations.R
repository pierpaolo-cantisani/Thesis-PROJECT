...
for(METHOD in 1:5){
  ...
  DM_sites <- read.csv(sprintf(file.path(PATH, "Dataset_0", "3_Benchmark", "DM_sites_Met%d.csv"), METHOD))
  ...
  if (METHOD %in% c(1, 3, 4)) {
    ...
    region_summary <- data.frame(
      ...
      DM_all = c(length(prom_genes), length(UTR5_genes), length(exon_genes),
                 length(intron_genes), length(UTR3_genes), length(interg_genes)),
      inters_DM = c(length(prom_inter_genes), length(UTR5_inter_genes), length(exon_inter_genes),
                    length(intron_inter_genes), length(UTR3_inter_genes), length(interg_inter_genes))
    )
    
    region_long <- region_summary %>%
      pivot_longer(cols = c(DM_all, inters_DM), names_to = "group", values_to = "n") %>%
      group_by(group) %>%
      mutate(pct = n / sum(n) * 100,
             group = factor(group, levels = c("DM_all", "inters_DM"),
                            labels = c("All DM genes", "DM ∩ DE genes")),
             region = factor(region, levels = c("Promoter","5'UTR","Exon","Intron","3'UTR","Intergenic")))
    
    region_colors <- c("Promoter"="#E41A1C","5'UTR"="darkgreen","Exon"="#FF7F00",
                       "Intron"="#4393C3","3'UTR"="lightgreen","Intergenic"="#984EA3")
    
    totals <- region_long %>% group_by(group) %>% summarise(total = sum(n), .groups = "drop")
    
    ttl <- if (METHOD == 1) "Method 1&2&5: Genomic region distribution"
    else sprintf("Method %d: Genomic region distribution", METHOD)
    
    g <- ggplot(region_long, aes(x = group, y = pct, fill = region)) +
      geom_bar(stat = "identity", width = 0.5) +
      geom_text(data = totals, aes(x = group, y = 102, label = paste0("n = ", total)),
      ...
      labs(title = ttl, subtitle = "All DM genes vs DM ∩ DE genes", x = NULL, y = "Percentage of genes") +
      theme_classic(base_size = 13) +
      theme(legend.position = "right", plot.title = element_text(face = "bold"),
            axis.text.x = element_text(size = 12))
    
    graph_list[[as.character(METHOD)]] <- g   # <-- chiave per NOME, non per indice
  }
}
